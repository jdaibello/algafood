...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private static final String END_USER_GENERIC_ERROR_MSG = "Ocorreu um erro interno inesperado no sistema. "
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
        ...
        Problem problem = createProblemBuilder(status, problemType, detail).userMessage(END_USER_GENERIC_ERROR_MSG)
                .build();
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
        ...
        if (body == null) {
            body = Problem.builder().timestamp(OffsetDateTime.now())
                    ...
                    .status(status.value()).userMessage(END_USER_GENERIC_ERROR_MSG).build();
        } else if (body instanceof String) {
            body = Problem.builder().timestamp(OffsetDateTime.now()).title((String) body).status(status.value())
                    .userMessage(END_USER_GENERIC_ERROR_MSG).build();
        }
